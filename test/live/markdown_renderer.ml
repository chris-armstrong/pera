...
let rec render_inline buf = function
  | Cmarkit.Inline.Text (text, _) -> add buf text
  ...
  | Cmarkit.Inline.Code_span (cs, _) ->
      add buf yellow;
      add buf "`";
      add buf (Cmarkit.Inline.Code_span.code cs);
      add buf "`";
      add buf reset
  | Cmarkit.Inline.Break (br, _) -> (
      ...
      | `Hard -> nl buf
      | `Soft -> add buf " ")
  | Cmarkit.Inline.Inlines (inlines, _) -> List.iter (render_inline buf) inlines
