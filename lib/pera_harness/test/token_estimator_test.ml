...
let test_estimate_message_includes_overhead () =
  ...
  if estimate < 4 then
    Alcotest.failf "expected estimate >= 4 (per_message_overhead), got %d" estimate
...
let test_estimate_messages_monotonic () =
  ...
  if both <= single then
    Alcotest.failf
      "expected estimate_messages to increase when appending a message: %d <= %d"
      both single
...
let test_estimate_counts_tool_call_arguments () =
      ...
      Pera_types.Types.
        {
          ...
          stop_reason = ToolUse;
          provenance =
            { api = "anthropic"; provider = "Anthropic"; model = "claude"; error_message = None };
          usage =
            {
              input_tokens = 0;
              output_tokens = 0;
  ...
  in
  let empty_estimate = Token_estimator.estimate_message (make_tool_call_msg empty_args) in
  let long_estimate = Token_estimator.estimate_message (make_tool_call_msg long_args) in
  if long_estimate <= empty_estimate then
    Alcotest.failf
      "expected long args estimate (%d) > empty args estimate (%d)"
...
let () =
  Alcotest.run "token_estimator"
    [
      ( "estimate_text",
        [
          Alcotest.test_case "ceil_div_3" `Quick test_estimate_text_is_ceil_div_3;
        ] );
      ( "estimate_message",
        [
          Alcotest.test_case "includes_overhead" `Quick test_estimate_message_includes_overhead;
