...
let test_generate_returns_36_chars () =
  let id = Pera_harness.Entry_id.(generate () |> to_string) in
...
let test_generate_returns_lowercase_hex_and_hyphens () =
  ...
  in
  Alcotest.(check bool)
    "only lowercase hex and hyphens" true
  ...
  Alcotest.(check bool)
    "variant nibble is [89ab]" true
    (List.mem ~eq:Char.equal variant [ '8'; '9'; 'a'; 'b' ])

let test_generate_ids_are_unique () =
  let ids =
    List.init 1000 (fun _ -> Pera_harness.Entry_id.(generate () |> to_string))
  in
...
let () =
  Alcotest.run "entry_id"
    [
      ( "entry_id",
        [
          Alcotest.test_case "returns 36 chars" `Quick
            test_generate_returns_36_chars;
          Alcotest.test_case "lowercase hex and hyphens" `Quick
            test_generate_returns_lowercase_hex_and_hyphens;
          Alcotest.test_case "1000 ids are unique" `Quick
            test_generate_ids_are_unique;
          Alcotest.test_case "lexicographic order over time" `Quick
            test_generate_ids_are_lexicographically_ordered_over_time;
        ] );
    ]
