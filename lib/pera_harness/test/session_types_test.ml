...
let make_provenance () =
  Pera_types.Types.
    { api = "anthropic"; provider = "anthropic"; model = "test-model"; error_message = None }

let make_assistant_message ?(stop_reason = Pera_types.Types.EndTurn) ?(cost_usd = None)
    content =
...
let test_session_info_omits_parent_session_id_when_none () =
  ...
  in
  let json = entry_to_json e in
  Alcotest.(check bool) "no parent_session_id key" false (has_key json "parent_session_id")
...
let test_assistant_message_stop_reason_strings () =
  let check_stop_reason sr expected =
    ...
    let json = message_to_json msg in
    Alcotest.(check string) ("stop_reason " ^ expected) expected (get_str json "stop_reason")
  in
