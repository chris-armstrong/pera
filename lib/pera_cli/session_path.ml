let generate_filename ~secure_random ~clock =
  let buf = Bytes.create 16 in
  secure_random buf;
  let uuid = Uuidm.v4 buf in
  let t = Unix.gmtime (Eio.Time.now clock) in
...
let resolve ~session_override ~session_dir ~secure_random ~clock =
  match session_override with
  | Some p -> p
  | None ->
      let fname = generate_filename ~secure_random ~clock in
