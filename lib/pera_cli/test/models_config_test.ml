...
let test_round_trip () =
  ...
  Alcotest.(check bool) "round-trip equal"
...
let test_compat_defaults () =
  ...
  | [p] ->
      (match p.compat with
      | Some c ->
          Alcotest.(check (option string)) "reasoning_field"
            (Some "reasoning_effort") c.reasoning_field;
          Alcotest.(check (option string)) "max_tokens_field"
            None c.max_tokens_field;
          Alcotest.(check (option bool)) "require_tool_result_name"
            None c.require_tool_result_name;
          Alcotest.(check (option string)) "enable_thinking_field"
            None c.enable_thinking_field
...
let () =
  Alcotest.run "models_config" [ ("models_config", suite) ]
