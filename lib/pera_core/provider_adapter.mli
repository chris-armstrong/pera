(** Provider adapter — binds a {!Pera_provider.Provider_registry.t} (and env +
...
val create :
  registry:Pera_provider.Provider_registry.t ->
  env:Eio_unix.Stdenv.base ->
  sw:Eio.Switch.t ->
  t
(** [create ~registry ~env ~sw] iterates the registry. For each
    [(api_name, (module P : Provider.S))]:
...
(** [stream_fn adapter] returns an {!Agent_types.stream_fn} that dispatches by
    ...
    Unknown [model.api] values produce an {!Pera_provider.Event_stream.t} that
