...
(** {1 Test helpers — shared with agent_loop_test} *)

(** Build a minimal [assistant_message] with the given stop_reason and content.
*)
...
(** Build an [assistant_message] with a single text block and EndTurn. *)
let make_text_assistant_message text =
  make_assistant_message [ Pera_types.Types.AText text ]

(** Build an [assistant_message] with tool calls and ToolUse stop_reason. *)
...
(** Build a Faux script for a turn that issues tool calls. *)
...
(** Build a schema requiring a field named "x" of type integer. *)
