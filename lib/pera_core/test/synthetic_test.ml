...
(** Pure Alcotest tests for Agent_types synthetic messages and compaction
    events. No Eio — these are all pure computations. *)
...
let test_agent_message_equal_synthetic () =
  ...
  Alcotest.(check bool)
    "equal Compaction_summary values are equal" true
    (Agent_types.agent_message_equal (Agent_types.Synthetic s1)
       (Agent_types.Synthetic s2));
  Alcotest.(check bool)
    "different Compaction_summary values are not equal" false
    (Agent_types.agent_message_equal (Agent_types.Synthetic s1)
       (Agent_types.Synthetic s3))
...
let test_agent_message_equal_mixed_is_false () =
  ...
  let synth =
    Agent_types.Synthetic (Agent_types.Compaction_summary { summary = "hello" })
  in
  Alcotest.(check bool)
    "Real vs Synthetic is false" false
    (Agent_types.agent_message_equal real synth);
  Alcotest.(check bool)
    "Synthetic vs Real is false" false
    (Agent_types.agent_message_equal synth real)
...
let test_equal_agent_event_compaction () =
  ...
  Alcotest.(check bool)
    "AE_compaction_end{a} equals itself" true
    (Agent_types.equal_agent_event e1 e2);
  Alcotest.(check bool)
    "AE_compaction_end{a} differs from {b}" false
    (Agent_types.equal_agent_event e1 e3);
  Alcotest.(check bool)
    "AE_compaction_end differs from AE_compaction_start" false
    (Agent_types.equal_agent_event e1 e_start)
...
let test_equal_agent_event_compaction_error () =
  ...
  Alcotest.(check bool)
    "AE_compaction_error equals itself" true
    (Agent_types.equal_agent_event e1 e2);
  Alcotest.(check bool)
    "AE_compaction_error differs on message" false
    (Agent_types.equal_agent_event e1 e3);
  Alcotest.(check bool)
    "AE_compaction_error differs from AE_compaction_start" false
    (Agent_types.equal_agent_event e1 Agent_types.AE_compaction_start)
