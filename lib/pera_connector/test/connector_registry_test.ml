...
let rec collect_events stream acc =
  match Pera_connector.Event_stream.take stream with
...
let test_lookup_returns_none_for_empty_registry () =
  let registry = Connector_registry.empty in
  match Connector_registry.lookup registry ~api:"anthropic" with
  | None -> ()
...
let test_register_is_pure_does_not_mutate () =
  let scripts = [ make_script "hello" ] in
  let provider_mod = Pera_core_test_util.Faux_provider.as_provider scripts in
  let original = Connector_registry.empty in
  let _new_registry =
    Connector_registry.register original ~name:"faux" provider_mod
  in
  (* original should be unchanged *)
  match Connector_registry.lookup original ~api:"faux" with
  | None -> ()
  | Some _ ->
      Alcotest.fail
...
let test_double_register_keeps_first_wins () =
  ...
  let registry =
    Connector_registry.empty |> fun r ->
    Connector_registry.register r ~name:"faux" provider1 |> fun r ->
    Connector_registry.register r ~name:"faux" provider2
  in
  match Connector_registry.lookup registry ~api:"faux" with
  | None ->
...
let test_to_list_exposes_full_registry_contents () =
  ...
  let registry =
    Connector_registry.empty |> fun r ->
    Connector_registry.register r ~name:"alpha" provider1 |> fun r ->
    Connector_registry.register r ~name:"beta" provider2
  in
  let entries = Connector_registry.to_list registry in
...
let () =
  Alcotest.run "Connector_registry"
    [
      ( "lookup",
        [
