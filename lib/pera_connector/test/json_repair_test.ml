...
let test_repair_invalid_backslash_escape () =
  ...
  let repaired = Pera_connector.Json_repair.repair input in
  let json =
...
let test_parse_streaming_returns_ok_on_valid_json () =
  let result = Pera_connector.Json_repair.parse_streaming (Some {|{"k":1}|}) in
...
let test_parse_streaming_repairs_and_returns_ok () =
  ...
  let result = Pera_connector.Json_repair.parse_streaming input in
  match result with
