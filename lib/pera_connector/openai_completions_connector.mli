(** OpenAI-completions connector implementation.
    ...
    The API key must be provided via the [~api_key] argument to {!create}.
    A convenience wrapper {!create_from_env} reads the key from the
...
(** [create ~api_key ~env ~sw] initialises an OpenAI connector with the
    given API key. *)
...
val create_from_env :
  ...
  sw:Eio.Switch.t ->
  (t, string) result
(** [create_from_env ~env ~sw] reads [OPENAI_API_KEY] from the environment
...
val stream_simple :
  ...
  sw:Eio.Switch.t ->
  (Pera_types.Types.assistant_message_event, Pera_types.Types.assistant_message)
  Event_stream.t
