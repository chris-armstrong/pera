...
Entry IDs are UUIDv7 truncated to 8 characters. UUIDv7's timestamp prefix means lexicographic id order matches creation order across processes. Truncation is for human readability; collision is checked at allocation.
...
**Single-flight.** A second `send` while one is running raises. Without this, concurrent prompts would interleave events into the same subscribers, which no consumer is designed to handle.
